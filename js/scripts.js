...
function updateResult() {
    document.getElementById('resultSpan').innerText = buttonClickHandler();
}

function buttonClickHandler() {
    // Retrieve input values
    var distance = parseInt(document.getElementById('input-range').value);
    var mode = document.getElementById('input-mode').value;
    var groupSize = parseInt(document.getElementById('input-size').value);
    var duration = parseInt(document.getElementById('input-days').value);

    // Hypothetical emission factors (replace with actual values)
    var emissionFactors = {
        air: 0.2,       // kg CO2 per km per passenger
        ship: 0.1,      // kg CO2 per km per passenger
        car: 0.15,      // kg CO2 per km per passenger
        bicycle: 0,     // Assumes zero emissions
        twowheler: 0.1, // kg CO2 per km per passenger
        train: 0.05,    // kg CO2 per km per passenger
        walk: 0         // Assumes zero emissions
    };

    // Calculate total emissions based on distance, mode, and group size
    var totalEmission = distance * emissionFactors[mode] * groupSize;

    // Calculate total emissions for the entire duration of stay
    totalEmission *= duration;

	// Display the result
	return (totalEmission.toFixed(2))/1000;

}


function updateResult2() {
    document.getElementById('resultSpan2').innerText = buttonClickHandler2();
}
...
function buttonClickHandler2() {
	...
	// Display the result
	return ((totalEmission)*7/1000).toFixed(2)

}

function updateResult3() {
    document.getElementById('resultSpan3').innerText = buttonClickHandler3();
}
...
function buttonClickHandler3() {
	...
	// Display the result
	return ((totalEmission)*7*101/1000).toFixed(1);
}



function updateResult11() {
    document.getElementById('resultSpan11').innerText = buttonClickHandler111();
}

function buttonClickHandler111() {
    // Retrieve input values
    var distance = parseInt(document.getElementById('input-range').value);
    var mode = document.getElementById('input-mode').value;
    var groupSize = parseInt(document.getElementById('input-size').value);
	...
	// Display the result
	return ((totalEmission)*0.45/1000).toFixed(2);

}


function updateResult22() {
    document.getElementById('resultSpan22').innerText = buttonClickHandler222();
}

function buttonClickHandler222() {
    // Retrieve input values
    var distance = parseInt(document.getElementById('input-range').value);
    var mode = document.getElementById('input-mode').value;
    var groupSize = parseInt(document.getElementById('input-size').value);
	...
	// Display the result
	return ((totalEmission)*0.45*6/1000).toFixed(2)

}

function updateResult33() {
    document.getElementById('resultSpan33').innerText = buttonClickHandler333();
}

function buttonClickHandler333() {
    // Retrieve input values
    var distance = parseInt(document.getElementById('input-range').value);
    var mode = document.getElementById('input-mode').value;
    var groupSize = parseInt(document.getElementById('input-size').value);
	...
	// Display the result
	return ((totalEmission)*0.45*5*101/1000).toFixed(1);
}
...
function handleSwitchChange(event) {
	if (event.target.value === 'yearly') {
	  yourFunction();triggerFunction(); }
  }
  var radioButtons = document.querySelectorAll('.radio-switch__input'); 
  radioButtons.forEach(function(radioButton) {
	radioButton.addEventListener('change', handleSwitchChange);
  });
  function yourFunction() {
	buttonClickHandler111();updateResult11();buttonClickHandler222();updateResult22();buttonClickHandler333();updateResult33();
  }
...
function triggerFunction() {
  var treeCountDiv = document.getElementById('treeCount');
  treeCountDiv.style.display = 'block'; // Make the div visible
  // You can add more styling or classes here if needed
}